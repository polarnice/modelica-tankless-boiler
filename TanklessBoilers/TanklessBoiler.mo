...
model TanklessBoiler "Tankless boiler model with control inputs"
  ...
  // Fluid package
  replaceable package Medium = Modelica.Media.Water.ConstantPropertyLiquidWater constrainedby Modelica.Media.Interfaces.PartialMedium "Fluid medium";
  // Conversion constants
  constant Real kelvinOffset = 273.15 "Kelvin temperature offset";
  constant Real btuToJoules = 1055.06 "BTU to Joules conversion factor";
  constant Real gallonsToM3 = 0.00378541 "Gallons to cubic meters";
  constant Real gpmToKgPerS = 0.06309 "GPM to kg/s conversion for water";
  constant Real feetToMeters = 0.3048 "Feet to meters";
  constant Real inchesToMeters = 0.0254 "Inches to meters";
  constant Real fToCScale = 5.0 / 9.0 "Fahrenheit to Celsius scale factor";
  constant Real fToCOffset = 32.0 "Fahrenheit to Celsius offset";
  ...
  parameter Real tAmbientF = 68.0 "Ambient temperature (°F) - for natural cooling (e.g., basement)" annotation(Dialog(group = "Temperature"));
  parameter Real mFlowGpm = 5.0 "Primary loop flow rate (GPM)" annotation(Dialog(group = "Primary Loop"));
  parameter Real primaryLoopVolumeGal = 2.0 "Primary loop volume (gallons) - total water volume in closed primary loop" annotation(Dialog(group = "Primary Loop"));
  // Internal parameters (SI units)
  parameter SI.HeatFlowRate Q_max = qMaxKbtu * 1000 * btuToJoules / 3600 "Maximum heat output (W)";
  parameter SI.HeatFlowRate Q_min = qMinKbtu * 1000 * btuToJoules / 3600 "Minimum heat output (W)";
  ...
  parameter SI.Temperature T_ambient = (tAmbientF - fToCOffset) * fToCScale + kelvinOffset "Ambient temperature (K)";
  parameter SI.Volume primaryLoopVolume = primaryLoopVolumeGal * gallonsToM3 "Primary loop volume (m³) - total water volume in closed primary loop";
  parameter SI.Density rho_nominal = Medium.density_pTX(Medium.p_default, Medium.T_default, Medium.X_default) "Nominal water density (kg/m3)";
  parameter SI.MassFlowRate m_flow_nominal = mFlowGpm * gpmToKgPerS "Nominal mass flow rate (kg/s) = GPM * 0.06309";
  parameter SI.VolumeFlowRate V_flow_nominal = m_flow_nominal / rho_nominal "Nominal volume flow rate (m3/s)";
  parameter Real head_ft = defaultHeadFt "Nominal pump head (feet)" annotation(Dialog(group = "Primary Loop"));
  parameter SI.Position head_nominal = head_ft * feetToMeters "Nominal pump head (m)";
  parameter Real eta = defaultEta "Efficiency (dimensionless) = 96% AFUE" annotation(Dialog(group = "Physical Properties"));
  parameter Real length_in = defaultLengthIn "Boiler pipe length (inches)" annotation(Dialog(group = "Physical Properties"));
  parameter Real diameter_in = defaultDiameterIn "Boiler pipe diameter (inches)" annotation(Dialog(group = "Physical Properties"));
  parameter Integer nNodes = 3 "Number of nodes for thermal mass" annotation(Dialog(group = "Physical Properties"));
  // Internal SI parameters for physical dimensions
  parameter SI.Length length = length_in * inchesToMeters "Boiler pipe length (m)";
  parameter SI.Diameter diameter = diameter_in * inchesToMeters "Boiler pipe diameter (m)";
  // External fluid connectors for secondary loop
  Modelica.Fluid.Interfaces.FluidPort_a port_a(redeclare package Medium = Medium) "Supply port (hot water out to secondary loop)" annotation(Placement(transformation(extent = {{90, -10}, {110, 10}})));
  Modelica.Fluid.Interfaces.FluidPort_b port_b(redeclare package Medium = Medium) "Return port (cool water back from secondary loop)" annotation(Placement(transformation(extent = {{-110, -10}, {-90, 10}})));
  // Control input
  Modelica.Blocks.Interfaces.BooleanInput enable "On/off signal (true=on, false=off)" annotation(Placement(transformation(extent = {{-140, -20}, {-100, 20}})));
  // Outputs (for monitoring)
  Modelica.Blocks.Interfaces.RealOutput Q_actual "Actual heat output (W)" annotation(Placement(transformation(extent = {{100, -30}, {120, -10}})));
  Modelica.Blocks.Interfaces.RealOutput m_flow "Mass flow rate (kg/s)" annotation(Placement(transformation(extent = {{100, -50}, {120, -30}})));
  Modelica.Blocks.Interfaces.RealOutput p_PSI "Pressure after pump (PSI)" annotation(Placement(transformation(extent = {{100, -70}, {120, -50}})));
